...
import org.apache.kafka.streams.kstream.internals.onetomany.SimpleKTableRepartitionerProcessorSupplier;
import org.apache.kafka.streams.kstream.internals.onetomany.SimpleKeyPartitioner;
...
//    }
    ...
    private <K0, V0, KO, VO> KTable<K0, V0> buildOneToManyJoin(KTable<KO, VO> other,
        ...
        KTableKTableRangeJoin<K0, V0, K, V, VO> joinByRange = new KTableKTableRangeJoin<K0, V0, K, V, VO>(joinThis.valueGetterSupplier(), joiner, joinPrefixFaker, rightKeyExtractor);
        ...
        //TODO - START - Added Experimental below to pipe to a topic.

        String myThisRepartitionSink = "myThisRepartitionSink";
        String myThisRepartitionTopicName = myThisRepartitionSink + "-Topic";
        topology.addInternalTopic(myThisRepartitionTopicName);

        myThis.toStream().to(myThisRepartitionTopicName, Produced.with(joinKeySerde, joinValueSerde));

        Materialized myThisRereadTopicStore = Materialized.<K0, V0, KeyValueStore<Bytes, byte[]>>as("myThisRereadTopicStore")
                .withCachingDisabled()
                .withLoggingDisabled()
                .withKeySerde(joinKeySerde)
                .withValueSerde(joinValueSerde);

        MaterializedInternal<K0, V0, KeyValueStore<Bytes, byte[]>> internalThisRereadTopicStore = new MaterializedInternal(myThisRereadTopicStore, builder, REPARTITION_NAME);

        KTable<K0, V0> myRereadThis =
                builder.table(myThisRepartitionTopicName,
                        new ConsumedInternal<>(joinKeySerde, joinValueSerde, new FailOnInvalidTimestamp(), null),
                        internalThisRereadTopicStore);

        //TODO - END - Added Experimental below to pipe to a topic.

        KTableImpl<K0, V, V0> myThat = new KTableImpl<>(builder, joinByRangeName, joinByRange, ((KTableImpl<K, ?, ?>) other).sourceNodes,
                ((KTableImpl<K, ?, ?>) other).queryableStoreName, false);
        //TODO - START - Added Experimental below to pipe to a topic.

        String myThatRepartitionSink = "myThatRepartitionSink";
        String myThatRepartitionTopicName = myThatRepartitionSink + "-Topic";
        topology.addInternalTopic(myThatRepartitionTopicName);

        myThat.toStream().to(myThatRepartitionTopicName, Produced.with(joinKeySerde, joinValueSerde));

        Materialized myThatRereadTopicStore = Materialized.<K0, V0, KeyValueStore<Bytes, byte[]>>as("myThatRereadTopicStore")
                .withCachingDisabled()
                .withLoggingDisabled()
                .withKeySerde(joinKeySerde)
                .withValueSerde(joinValueSerde);

        MaterializedInternal<K0, V0, KeyValueStore<Bytes, byte[]>> internalThatRereadTopicStore = new MaterializedInternal(myThatRereadTopicStore, builder, REPARTITION_NAME);

        KTable<K0, V0> myRereadThat =
                builder.table(myThatRepartitionTopicName,
                        new ConsumedInternal<>(joinKeySerde, joinValueSerde, new FailOnInvalidTimestamp(), null),
                        internalThatRereadTopicStore);
...
//        MaterializedInternal<K0, V0, KeyValueStore<Bytes, byte[]>> myUselessMaterializedStore = new MaterializedInternal(myMat, builder, "SOME_HANDLE_NAME");
        ...
        final KTableKTableJoinMerger<K0, V0> joinMerge = new KTableKTableJoinMerger<>(
                ((KTableImpl<K0, ?, V0>) myRereadThis),
                ((KTableImpl<K0, ?, V0>) myRereadThat),
            internalQueryableName);
...
//                myUselessMaterializedStore.storeName());
        ...
        //Fixes: Processor KTABLE-MERGE-0000000006 is not added yet.
        topology.addProcessor(joinMergeName, joinMerge);



        topology.addProcessor("thisSource", ((KTableImpl<K0, ?, V0>) myRereadThis).processorSupplier);
        topology.addProcessor("thatSource", ((KTableImpl<K0, ?, V0>) myRereadThat).processorSupplier);
        topology.connectProcessors(joinMergeName, "thisSource");
        topology.connectProcessors(joinMergeName, "thatSource");

//        topology.connectProcessorAndStateStores(joinMergeName, myRereadThat.queryableStoreName());
//        topology.connectProcessorAndStateStores(joinMergeName, myRereadThis.queryableStoreName());


        //Did not work: StateStore SomeStore is not added yet.
//        topology.connectProcessorAndStateStores(((AbstractStream<K>) myRereadThis).name, internalQueryableName);
//        topology.connectProcessorAndStateStores(((AbstractStream<K>) myRereadThat).name, internalQueryableName);

        //Did not work to provide joinMerge
//        topology.connectProcessorAndStateStores(joinMergeName, myRereadThat.queryableStoreName());
//        topology.connectProcessorAndStateStores(joinMergeName, myRereadThis.queryableStoreName());

        //Did not work to provide joinMerge
        //topology.connectProcessors(joinMergeName, ((AbstractStream<K>) myRereadThis).name);
        //topology.connectProcessors(joinMergeName, ((AbstractStream<K>) myRereadThat).name);

        //Did not work to provide joinMerge
        //topology.connectProcessors(((AbstractStream<K>) myRereadThis).name, joinMergeName);
        //topology.connectProcessors(((AbstractStream<K>) myRereadThat).name, joinMergeName);


        final Set<String> allSourceNodes = ensureJoinableWith((AbstractStream<K>) other); //TODO Unsafe probably... sigh.
//        topology.addStateStore(new KeyValueStoreMaterializer<>(internalThatRereadTopicStore).materialize(), internalThatRereadTopicStore.storeName());
//        topology.addStateStore(new KeyValueStoreMaterializer<>(internalThisRereadTopicStore).materialize(), internalThisRereadTopicStore.storeName());



        //topology.connectProcessorAndStateStores(joinMergeName, joinMerge);
...
//            .to(outputRepartitionSinkTopicName, Produced.with(joinKeySerde, joinValueSerde));
        ...
        sourcesNeedCopartitioning.add(myThatRepartitionTopicName);
        sourcesNeedCopartitioning.add(myThisRepartitionTopicName);
