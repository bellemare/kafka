...
public class CogroupedKStreamImplTest {
    ...
    @Before
    public void before() {
        final KStream<String, String> stream = builder.stream(TOPIC_1, Consumed.with(Serdes.String(), Serdes.String()));
        cogroupedStream = stream.groupByKey(Grouped.with(Serdes.String(), Serdes.String())).cogroup(MockAggregator.TOSTRING_ADDER);
    }
    @After
    public void cleanup() {
        if (driver != null) {
    ...
    public void shouldNotHaveNullAggregatorOnCogroup() throws Exception {
        cogroupedStream.cogroup(builder.stream(TOPIC_2, Consumed.with(Serdes.String(), Serdes.String())).groupByKey(), null);
    }
    @Test(expected = NullPointerException.class)
    public void shouldNotHaveNullInitializerOnAggregate() throws Exception {
        cogroupedStream.aggregate(null, Serdes.String(), Materialized.with(Serdes.String(), Serdes.String()));
    }
    @Test(expected = NullPointerException.class)
    ...
    public void shouldNotHaveInvalidStoreNameOnAggregate() throws Exception {
        cogroupedStream.aggregate(MockInitializer.STRING_INIT, Serdes.String(), Materialized.as(INVALID_STORE_NAME).with(Serdes.String(), Serdes.String()));
    }
//    @Test(expected = NullPointerException.class)
//    public void shouldNotHaveNullInitializerOnWindowedAggregate() throws Exception {
//        cogroupedStream.aggregate(null, TimeWindows.of(10), Serdes.String(), Materialized.as("store").with(Serdes.String(), Serdes.String()));
//    }
//    @Test(expected = NullPointerException.class)
