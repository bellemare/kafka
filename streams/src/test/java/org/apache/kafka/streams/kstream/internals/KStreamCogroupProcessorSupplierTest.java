...
public class KStreamCogroupProcessorSupplierTest {
    ...
    private final KStreamCogroupProcessorSupplier<String, Long> cogroup = new KStreamCogroupProcessorSupplier<String, Long>(Collections.singleton(parent));
    private final Processor<String, Change<Long>> processor = cogroup.get();
    private InternalMockProcessorContext context;
    private List<KeyValue> results = new ArrayList<>();
    @Before
    public void setup() {
        context = new InternalMockProcessorContext(null, Serdes.String(), Serdes.Long(), new NoOpRecordCollector(), new ThreadCache(new LogContext("testCache"), 100000, new MockStreamsMetrics(new Metrics()))) {
            @Override
